...
class MyWindow(QMainWindow, form_class):
    ...
    def start_trading(self):
        ...
        self.trade_timer.start(1000 * 10)  # 7초마다 trade_stocks 호출
    ...
    def trade_stocks(self):
        ...
        for code in codes:
            if code.strip():  # 종목 코드가 비어 있지 않은 경우에만 처리
                # 현재가 조회 및 로그 출력
                current_price_info = self.kiwoom.block_request("opt10001",
                                                          종목코드=code.strip(),
                                                          output="주식기본정보",
                                                          next=0)
                current_price_str = current_price_info['현재가'][0].replace(",", "").strip()
                
                # 현재가가 빈 문자열인 경우나 현재가 정보 조회 실패 시 처리
                if not current_price_str or not current_price_str.isdigit():
                    print(f"[{code.strip()}] 현재가 정보 조회 실패")
                    continue  # 다음 종목으로 넘어감
            
                current_price = abs(int(current_price_str))  # 현재가 절대값 변환
