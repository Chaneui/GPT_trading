...
class MyWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        # 슬랙 웹훅 URL 설정
        self.slack_webhook_url = "여기에_당신의_슬랙_웹훅_URL을_입력하세요"  
    ...
    def buy_stock(self, code, price, quantity):
        ...
        # 슬랙 메시지 전송 추가
        self.send_slack_message(f"[매수 주문] [{code}] [가격: {price}] [수량: {quantity}]")
    ...
    def sell_all_stocks(self):
        ...
        if '종목번호' in result and len(result['종목번호']) > 0:  # KeyError 방지 및 결과 데이터가 비어 있지 않은지 확인
            for i in range(len(result['종목번호'])):
                ...
                if quantity > 0:  # 보유 수량이 0보다 클 때만 매도 주문
                    ...
                    # 슬랙 메시지 전송 추가
                    self.send_slack_message(f"[매도 주문] [{code}] [시장가] [수량: {quantity}]")
                else:
                    self.buysell_log.append("[매도 주문 실패] 보유한 주식이 없습니다.")
                    self.send_slack_message("[매도 주문 실패] 보유한 주식이 없습니다.")
        else:
            self.buysell_log.append("매도 위해 가져온 종목 데이터 오류")
            self.send_slack_message("매도 위해 가져온 종목 데이터 오류")
        ...
        for single_date in (start_date + datetime.timedelta(n) for n in range(15)):
            base_date = single_date.strftime('%Y%m%d')
            
            df = self.kiwoom.block_request("opt10170",
                                        계좌번호=self.kiwoom.GetLoginInfo("ACCNO")[0],  # 첫 번째 계좌 사용
        ...
        self.buysell_log.append("매매일지.csv 파일 생성 완료")
